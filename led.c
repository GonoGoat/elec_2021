...
void main(void) {
    ...
    char mesuress[7]= {8, 9, 10, 11, 12, 13, 14};
    ...
    enregistrer_mesure(mesuress);
    
    char alim_off = 1;
    
    while(1){      
        PORTBbits.RB4 = !PORTBbits.RB4;
        PORTBbits.RB5 = !alim_off;
...
void enregistrer_mesure(char measurement_data[7]){
    ...
    // Mise � jour du nombre d'enregistrements
...
// ecrire une donn�e dans une case de l'eeprom
void EEPROM_Write(char address, char data){
...
// lire une donn�e d'une case de l'eeprom
char EEPROM_Read(char address){
...
// initialiser le compteur, n�cessaire en cas de mise hors tension
void initialisation_compteur(){
    char valeur_actuelle = EEPROM_Read(0);
    if(!valeur_actuelle >= 0 || !valeur_actuelle <=36){
        EEPROM_Write(0, 0); // Initionalisation à 0
    }
}